...
const CanvasEditorModal = ({ image, onClose }) => {
  ...
  useEffect(() => {
    ...
    img.src = image?.displayUrl?.startsWith("http")
      ? image.displayUrl
      : image?.permanentUrl?.startsWith("http")
        ? image.permanentUrl
        : "";

    if (!img.src) {
      console.error("CanvasEditorModal: Invalid image passed in", image);
      canvas.width = 512;
      canvas.height = 512;
    ...
    img.onerror = () => {
      console.error("CanvasEditorModal: Failed to load image", img.src);
      canvas.width = 512;
      canvas.height = 512;
      context.fillStyle = "red";
      context.font = "20px sans-serif";
  ...
  const ToolButton = ({ icon, label, toolName, children }) => (
    <div className="relative flex flex-col items-center group">
      ...
      {activeTool === toolName && children && (
        <div className="absolute left-full ml-4 p-4 bg-zinc-800 rounded-lg shadow-xl w-64 z-10">
          {children}
        </div>
      )}
    </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="bg-zinc-900 text-white w-full h-full md:max-w-6xl md:max-h-[90vh] md:rounded-lg shadow-xl flex flex-col">
        ...
        <div className="flex flex-grow min-h-0">
          <aside className="w-24 p-4 bg-zinc-800/50 border-r border-zinc-700 flex flex-col items-center gap-6">
            <ToolButton
            ...
            <button
              onClick={clearCanvas}
              className="flex flex-col items-center p-3 rounded-lg text-zinc-300 hover:bg-red-600 hover:text-white transition-colors duration-200"
            >
